...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const defaultFeatures = locale === 'en' ? defaultFeaturesEn : defaultFeaturesZh
  const defaultSpecs = locale === 'en' ? defaultSpecsEn : defaultSpecsZh
    ...
    <div className="py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-8">
          ...
          <ArrowRight className="w-4 h-4" />
          <span className="text-sky-500 font-medium">{product.title}</span>
        </nav>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <p className="text-lg text-gray-600 mb-6">{product.summary || product.description || t('product.defaultDesc')}</p>
